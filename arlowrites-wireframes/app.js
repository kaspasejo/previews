...
/* orphan control: join the last three words of running text with nbsp */
const eligible='p, blockquote, dd, li';
document.querySelectorAll(eligible).forEach(element=>{
  for(const node of element.childNodes){
    if(node.nodeType!==Node.TEXT_NODE||!node.textContent.trim())continue;
    node.textContent=node.textContent.replace(/\s+(\S+)\s+(\S+)\s*$/, ' $1 $2');
  }
});
